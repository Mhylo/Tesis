...
Expected: PASS ya, porque `roi.py` todavía no importa matplotlib en ningún sitio. **Es correcto y esperado**: esta prueba es una guarda contra la regresión que introduce el Step 3, no una prueba de algo que falte. Anótalo y sigue: lo que tiene que seguir pasando es el Step 4, después de añadir `elegir()`.
...
Expected: PASS, 21 pruebas. En particular `test_importar_roi_no_arrastra_matplotlib` **sigue** pasando: si falla, el `import matplotlib` se te quedó arriba del archivo.
...
**El Step 2 de la Task 3 es raro a propósito.** La prueba de matplotlib perezoso *pasa* antes de escribir el código, porque vigila una regresión que el Step 3 podría introducir. Está anotado en la tarea; no la "arregles".
