...
- [ ] **Step 3b: Alinear el estilo de la CLI de la ida con la de la vuelta**

La Task 5 escribió `CamposT/propagacion.py` como espejo de `retropropagacion.py`, pero se le cayeron las tildes en cuatro sitios que en el original sí las llevan. La simetría entre los dos módulos es un requisito, y una ayuda de `--help` que difiere de su espejo es exactamente lo que se nota al compararlas. Sustituye en `CamposT/propagacion.py`:

| línea | dice | debe decir |
|---|---|---|
| 26 | `UNIDADES: milimetros para todo.` | `UNIDADES: milímetros para todo.` |
| 54 | `...o um para todo.` | `...o µm para todo.` (micro, U+00B5, como `retropropagacion.py:148`) |
| 68 | `help="paso de pixel [mm]"` | `help="paso de píxel del sensor [mm]"` (idéntico al de la vuelta) |
| 104-106 | `raiz`, `modulo`, `dejaria`, `invocacion` | `raíz`, `módulo`, `dejaría`, `invocación` (el comentario es copia literal de `retropropagacion.py:195-197`) |

Ojo: sólo el **comentario** lleva tilde; la variable sigue llamándose `raiz`. Y esto no toca los mensajes de commit, que siguen siendo ASCII.
